	...
	where T : struct
{
	...
	public static Buffer16<T> FromSpan(ReadOnlySpan<T> values)
	{
		Buffer16<T> buffer = default;
		for (int i = 0; i < Math.Min(values.Length, _size); i++)
			buffer[i] = values[i];
		return buffer;
	}
	...
	where T : struct
{
	...
	public static Buffer32<T> FromSpan(ReadOnlySpan<T> values)
	{
		Buffer32<T> buffer = default;
		for (int i = 0; i < Math.Min(values.Length, _size); i++)
			buffer[i] = values[i];
		return buffer;
	}
