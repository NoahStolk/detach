...
public class TriangleSphereIntersectionTests
{
	...
	public void TestCollisionWithOriginTriangleFacingUp()
	{
		...
		// No collision.
		TestCollision(triangleAtOriginFacingUp, new(new(2.00f, 0.00f, 0.00f), 0.75f), null);
		TestCollision(triangleAtOriginFacingUp, new(new(0.00f, 1.75f, 0.00f), 0.75f), null);

		// Surface collision at 0,0,0.
		TestCollision(triangleAtOriginFacingUp, new(new(0.00f, 0.75f, 0.00f), 0.75f), new(default, default));

		// Spheres along the Y axis. Intersection point is at 0,0,0.
		TestCollision(triangleAtOriginFacingUp, new(new(0.00f, 0.50f, 0.00f), 1.00f), new(new(0.00f, 0.50f, 0.00f), default));
		TestCollision(triangleAtOriginFacingUp, new(new(0.00f, 0.50f, 0.00f), 0.75f), new(new(0.00f, 0.25f, 0.00f), default));
		TestCollision(triangleAtOriginFacingUp, new(new(0.00f, 0.75f, 0.00f), 1.00f), new(new(0.00f, 0.25f, 0.00f), default));
		TestCollision(triangleAtOriginFacingUp, new(new(0.00f, 0.99f, 0.00f), 1.00f), new(new(0.00f, 0.01f, 0.00f), default));

		// Offset the sphere at the X axis. Should still be inside the triangle.
		TestCollision(triangleAtOriginFacingUp, new(new(0.45f, 0.50f, 0.00f), 1.00f), new(new(0.00f, 0.50f, 0.00f), new(0.45f, 0.00f, 0.00f)));
		TestCollision(triangleAtOriginFacingUp, new(new(0.50f, 0.50f, 0.00f), 1.00f), new(new(0.00f, 0.50f, 0.00f), new(0.50f, 0.00f, 0.00f)));

		// Should not be inside the triangle anymore, so perform edge tests.
		TestCollision(triangleAtOriginFacingUp, new(new(0.55f, 0.50f, 0.00f), 1.00f), new(new(0.00f, 0.497506201f, 0.00f), new(0.50f, 0.00f, 0.00f)));
	}
	...
	private static void TestCollision(Triangle triangle, Sphere sphere, (Vector3 CollisionForce, Vector3 IntersectionPoint)? expectedIntersection)
	{
		...
		static void AssertVector3Equality(Vector3 expected, Vector3 actual)
		{
			...
			string message = $"Expected {expected.ToString("N4")}, actual {actual.ToString("N4")}";
